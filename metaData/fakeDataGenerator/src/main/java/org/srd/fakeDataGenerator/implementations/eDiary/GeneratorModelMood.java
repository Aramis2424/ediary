    ...
    public List<ModelMood> generate(Map<String, List<? extends Model>> context) {
        ...
        for (Model model : owners) {
            ...
            for (int i = 0; i < cntMoodsPerOwner; i++) {
                ...
                LocalDateTime bedtime = LocalDateTime.of(createdDate, Utils.getTimeBetweenHours(0, 4));
                LocalDateTime wakeUpTime = LocalDateTime.of(createdDate, Utils.getTimeBetweenHours(7, 10));

                out.add(new ModelMood(moodId, ownerId, scoreMood, scoreProductivity, bedtime, wakeUpTime, createdDate));
