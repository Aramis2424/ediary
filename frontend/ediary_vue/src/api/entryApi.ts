...
export const getPermissionEntry = async (diaryId: number, date: string): Promise<AxiosResponse<void>> => {
  const res: AxiosResponse<void> = await api.head(`/diaries/${diaryId}/entries`, {
    params: {
      date: date
    }
  });
